...
| 5 | **Credibility & the cost thesis** — is Fonderie *cheaper* in tokens, and can we prove it without grading our own homework? | Pilot run; **"fraction of token cost" not delivered — tracking parity-plus, not ≤⅓** | Pre-registered benchmark (thresholds locked before data), dual-method token attribution, per-session quality floors, guardrails that refuse a verdict on thin/contradictory data. The n=1 pilot lands at **parity-plus** (≈0.7× fat overhead), so by our own locked rule the "fraction" headline is **dropped**; the durable, measured claim is **correctness density** — scratch shipped auth *below* the security floor (no logout, no rate-limit) while Fonderie scored at ceiling in half the code. Next: a scoped-brain design (`pb-scoped`) attempts the ⅓ bar; if it can't, parity-plus + correctness-density is the honest claim | `fonderie-js/experiments/phase41-2026-07/PILOT-FINDINGS.md`; `BRAIN_PLAN.md` § Phase 4.1 |
