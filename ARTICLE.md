...
| 5 | **The cost thesis & credibility** — is Fonderie *cheaper* in tokens, and can we prove it without grading our own homework? | **Goal met (N=3): the token-cost fraction is real** — pending line-by-line quality sign-off | Pre-registered benchmark (thresholds locked before data), 36 clean sessions across 3 conditions × 3 sequences × a 4-session growing app. Transcript-measured Fonderie-knowledge overhead **pb/fat = 0.24 → a fraction (≤⅓)**, stable as N grew (0.13→0.22→0.24). A scare along the way — a confounded metric briefly read parity-plus — was traced (for $0, by instrumenting the transcripts we already had) to **turn-count noise**, which averaged out at N=3 (the fat skill and the brain take equal turns). Quality held equal between the Fonderie conditions (delegation-aware security score fat 8.0 ≈ brain 7.7) while from-scratch shipped real flaws — an insecure hardcoded secret in **2 of 3** runs — in **~3× the code**. Correctness density is measured, not asserted | `fonderie-js/experiments/phase41-2026-07/BATCH-RESULTS.md`; `BRAIN_PLAN.md` § Phase 4.1 |
