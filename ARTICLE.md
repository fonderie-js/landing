...
| 5 | **The cost thesis & credibility** — is Fonderie *cheaper* in tokens, and can we prove it without grading our own homework? | **Cost goal met (N=3); quality mostly-but-not-uniformly equal** | Pre-registered benchmark, 36 clean sessions (3 conditions × 3 sequences × a 4-session growing app). Transcript-measured Fonderie-knowledge overhead **pb/fat = 0.24 → a fraction (≤⅓)**, stable as N grew (0.13→0.22→0.24); a parity-plus scare was traced (for $0, by instrumenting transcripts we already had) to turn-count noise that averaged out at N=3. On quality the full 39-point rubric is honest about a wrinkle: the brain matched the fat skill on **2 of 3** sequences (39, 38 of 39) but **stalled on the third** — the model declined a feature it could have installed — a discovery-reliability gap now targeted. From-scratch was consistently *complete* but *insecure* (hardcoded secret in 2/3 runs) in ~3× the code. Nothing claimed beyond the measurement | `fonderie-js/experiments/phase41-2026-07/BATCH-RESULTS.md`; `BRAIN_PLAN.md` § Phase 4.1 |
